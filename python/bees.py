...
from utility import trim_tail, add, devision
...
class BEES(object):
        ...
        for itr in range(max_iter):
            ...
            for i in range(0, len(x), batch_size):
                ...
                for m in range(self.M):
                    ...
                    self.swams[m].fitness, _ = obj._evaluate(batch_x, batch_y, len(batch_x))
    ...
    def update_pool(self):
        print('Update Pool')
        mean_matrix = self.get_mean_matrix()
        #print(mean_matrix.shape, mean_matrix)
        for particle in self.swams:
    ...
    def update_learning_config(self, max_iter, itr):
        ...
        tmp_c1 = self.MAX_C1 - (self.MAX_C1 * itr / max_iter) + self.MIN_C1
        tmp_c2 = np.min([self.MIN_C2 + (self.MAX_C2 * itr / max_iter), self.MAX_C2])
        tmp_c3 = self.MAX_C3 - (self.MAX_C3 * itr / max_iter) + self.MIN_C3
        total = tmp_c1 + tmp_c2 + tmp_c3
        self.C1 = tmp_c1 / total
        self.C2 = tmp_c2 / total
        self.C3 = tmp_c3 / total
        print('\nItr: {}\nC1: {}, C2:{}, C3:{}'.format(itr, self.C1, self.C2, self.C3))

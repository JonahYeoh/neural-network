...
class GA(object):
    def __init__(self, config):
        ...
        self.idx_tracker = 0
        self.rr = config['radioactive_rating'][1]
        self.gf = config['grow_factor'][1]
        self.crossover_rate = config['x_rate'][1]
    ...
    def sampling(self):
        ...
        if len(unique_gene) == len(self.population):
            ...
            self.population.sort(key=lambda p: p.fitness + np.random.uniform(-shift, shift, 1), \
                reverse = self.aim)
            while len(self.population) < self.M:
    ...
    def crossover(self, seq1, seq2, method='uniform'): # single point, two points
